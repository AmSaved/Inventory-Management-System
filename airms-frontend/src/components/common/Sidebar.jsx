...
const Sidebar = ({ open }) => {
  ...
  const [expandedGroups, setExpandedGroups] = useState(['approvals', 'ops']);
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-50 w-72 transition-transform duration-300 ease-in-out border-r border-gray-200 bg-white ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
      <div className="h-full flex flex-col relative">
          ...
          <div className="p-8 border-b border-gray-100 relative z-10">
            ...
            >
              ...
              <div>
                <span className="text-xl font-black text-gray-900 tracking-tighter uppercase italic block leading-none">
                   {user?.organization_node?.name || 'AIRMS'}
                </span>
                <span className="text-[8px] font-black text-blue-600 uppercase tracking-[0.4em] mt-2 block opacity-90">
                   INSTITUTIONAL CORE
                </span>
              </div>
            </button>
          ...
          <nav className="flex-1 px-4 py-8 overflow-y-auto space-y-8 custom-scrollbar relative z-10">
            {majorCategories.map((major) => {
              ...
              const visibleGroups = major.groups.filter(group =>
                group.items.some(item => checkItemPermission(item.permissions))
              );
                ...
                <div key={major.id} className="space-y-4">
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-2 rounded-2xl transition-all duration-300 group ${
                      isMajorExpanded ? 'text-gray-900' : 'text-gray-400 hover:text-gray-700'
                    }`}
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-[10px] font-black tracking-[0.2em] uppercase">{major.name}</span>
                    </div>
                  ...
                  <AnimatePresence initial={false}>
                    {isMajorExpanded && (
                      <motion.div
                        ...
                        className="space-y-2 pl-4 border-l border-white/5 ml-6"
                      >
                        {visibleGroups.map((group) => {
                          const isGroupExpanded = expandedGroups.includes(group.id);
                          const visibleItems = group.items.filter(item => checkItemPermission(item.permissions));
                            ...
                            <div key={group.id} className="space-y-1">
                              <button
                                ...
                                className={`w-full flex items-center justify-between px-4 py-2 rounded-xl transition-all duration-300 ${
                                  isGroupExpanded ? 'text-blue-600 bg-blue-50/50' : 'text-gray-400 hover:text-gray-700 hover:bg-gray-50'
                                }`}
                              >
                                <span className="text-[9px] font-black tracking-widest uppercase">{group.name}</span>
                                <div className={`w-1.5 h-1.5 rounded-full transition-all duration-500 ${isGroupExpanded ? 'bg-blue-600 shadow-md' : 'bg-gray-200'}`} />
                              </button>
                                    ...
                                    {visibleItems.map((item) => {
                                      ...
                                      return (
                                        <NavLink
                                          ...
                                          to={item.href}
                                          className={`
                                            flex items-center gap-4 px-5 py-3 rounded-2xl text-[11px] transition-all duration-300 relative group/link
                                            ${active
                                        ...
                                        >
                                          ...
                                          <span className="tracking-wide font-bold uppercase">{item.name}</span>
                                          {active && (
